...
const getProtectedTrelloResource = (resourceUrl, req) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    settings.oauth.getProtectedResource(resourceUrl, "GET", accessToken, accessTokenSecret, (error, data, response) => {
      if (error) {
        return resolve({
...
router.get("/me", (req, res) => {
  getProtectedTrelloResource("https://api.trello.com/1/members/me", req).then((response) => res.status(response.statusCode).send(response.resource));
});

router.get("/my/boards", (req, res) => {
  getProtectedTrelloResource("https://trello.com/1/members/my/boards", req).then((response) =>
    res.status(response.statusCode).send(response.resource)
  );
});

router.get("/boards/:boardid/lists", (req, res) => {
  getProtectedTrelloResource(`https://trello.com/1/boards/${req.params.boardid}/lists`, req).then((response) =>
    res.status(response.statusCode).send(response.resource)
  );
});

router.get("/lists/:listid", (req, res) => {
  getProtectedTrelloResource(
    `https://trello.com/1/lists/${req.params.listid}/cards/?actions=updateCard:idList,createCard,copyCard,convertToCardFromCheckItem&filter=all&fields=name,labels,actions`,
    req
